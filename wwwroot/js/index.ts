...
function updateGrid(): void {
    ...
    if (buttonGrid) {
        ...
        const buttons: NodeListOf<HTMLButtonElement> = document.querySelectorAll('.button-grid button') as NodeListOf<HTMLButtonElement>;

        buttons.forEach((button, index) => {
            // Clone the button to remove existing event listeners
            const newButton: HTMLButtonElement = button.cloneNode(true) as HTMLButtonElement;
